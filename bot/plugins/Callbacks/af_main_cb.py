...
async def cb_navg(bot, update: CallbackQuery):
    ...
    if not int(index_val) == 0:    
        temp_results.append([
            InlineKeyboardButton(f"🎖️Page {index_val + 1}/{len(results) if len(results) < max_pages else max_pages} 🎖️", callback_data="ignore")
        ])
    ...
    text=f"<i>🎖️Check links At @CC_Linkz🎖️..Found</i> <code>{leng}</code> <i>Results For Your Query:</i> <code>{query}</code>"
...
async def callback_data(bot, update: CallbackQuery):
    ...
    if query_data == "start":
        buttons = [[
            InlineKeyboardButton('My Dev 👨‍🔬', url='https://t.me/Harshad_SCam'),
        ],[
            InlineKeyboardButton('Support✔️', url='https://t.me/CV_Movies')
        ],[
